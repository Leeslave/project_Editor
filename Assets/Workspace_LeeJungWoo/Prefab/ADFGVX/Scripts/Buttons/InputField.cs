...
public class InputField : Button
{
    ...
    public virtual void AddInputFieldByKeyboard(string value)//입력창에 키보드로 입력
    {
        ...
        {
            adfgvx.InformError(InputFieldName + " 최대 입력 : 입력 불가");
            return;
        }
    ...
    public virtual void AddInputFieldByButton(string value)//입력창에 버튼으로 입력
    {
        if (inputString.Length > InputFieldMaxLength)
        {
            adfgvx.InformError(InputFieldName + " 최대 입력 : 입력 불가");
    ...
    public virtual void DeleteInputFieldByKeyboard(int length)//입력창에 키보드로 삭제
    {
        if (!GetIsReadyForInput())
            return;
        ...
        {
            adfgvx.InformError(InputFieldName + " 최소 입력 : 삭제 불가");
            return;
        }
    ...
    public virtual void DeleteInputFieldByButton(int length)//입력창에 버튼으로 삭제
    {
        if (inputString.Length == 0)
        {
            adfgvx.InformError(InputFieldName + " 최소 입력 : 삭제 불가");
    ...
    public virtual void ClearInputFieldByButton()//입력창에 버튼으로 비움
    {
        inputString = "";
        SetMarkText("");
    }

    public virtual void ReturnInputFieldByKeyboard()//검색창에 키보드로 엔터
    {
        if (!GetIsReadyForInput())
            return;
    ...
    public virtual void DisplayErrorInInputField(string value)//검색창에 에러 메세지를 띄운다
    {
        SetMarkText(value);
        inputString = "";
        isReadyForInput = false;
    ...
    public virtual void StopFlashInputField()//검색창 선택과 깜박임을 비활성화한다
    {
        SetMarkText(inputString);
        isReadyForInput = false;
        isFlash = false;
